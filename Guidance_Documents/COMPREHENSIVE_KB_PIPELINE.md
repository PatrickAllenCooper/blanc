...
**Paper file**: `paper/paper.tex` (the single NeurIPS 2026 E&D submission — all other .tex files are companions or archived)

**Paper consolidation (2026-05-04)**: The TMLR-targeted `paper.tex` (2745 lines) has been archived to `paper/paper_tmlr_archive.tex`. The NeurIPS-formatted `paper/dataset_paper.tex` has been absorbed and deleted. The single submission is now `paper/paper.tex`, using `\usepackage[eandd, nonanonymous]{neurips_2026}`, containing a full 9-section main body (21 pages total with appendices, clean compile) with:
- Six numbered contributions: pipeline, cross-ontology KB, synthetic contamination control, baseline evaluation, M5 multimodal, and DeFAb as finetuning substrate
- Six cohesive figures: TikZ pipeline (Fig.1), IDP Level-3 storyboard (Fig.2), provenance timeline + composition (Fig.3), 3-panel results grid with symbolic ceiling (Fig.4), difficulty stratification (Fig.5, appendix), verifier-as-reward (Fig.6)
- "Worst results" framing leading the evaluation section: symbolic ceiling ASP=100%, rendering-robust 7.8-23.5%, L3 direct near-zero, Kimi 80.9% decoder failure
- DeFAb as finetuning substrate section (§7): DPO, RLVR/GRPO, conflict games, sc2live verifier-gated commander

The old TMLR paper's full formal machinery, fine-tuning pipeline depth (DPO/RLHF/GRPO/MCTS), and adversarial debate protocol are archived in `paper/paper_tmlr_archive.tex` for future TMLR submission.

**Figure system**: All matplotlib figures generated by `paper/generate_figures.py` using the 5-color design system (defabBlue #1F3A6B, defabTeal #3D6B7A, defabGold #D9A441, defabRed #B0413E, defabGray #6B7280). TikZ figures (`fig_pipeline.tex`, `fig_level3_example.tex`, `fig_finetuning.tex`) use matching `\definecolor` values.
...
**Maintained by**: Anonymous Authors  
