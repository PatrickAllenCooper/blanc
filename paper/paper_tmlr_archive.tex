...
% =====================================================================
% JOURNAL SUBMISSION FORMAT: Transactions on Machine Learning Research
% =====================================================================
% This manuscript targets TMLR (https://jmlr.org/tmlr/), the
% journal-of-record for substantive theoretical-empirical contributions
% in machine learning that exceed conference page budgets. TMLR is
% appropriate to this manuscript on three grounds: (1) length --- the
% paper develops a benchmark, formal framework, fine-tuning pipeline,
% adversarial debate protocol, and game-theoretic foundations across
% ~80 typeset pages with extensive appendices, exceeding any conference
% page budget; (2) scope --- TMLR explicitly accepts work that combines
% novel theoretical results with empirical evaluation, which matches
% this paper's structure; (3) open access --- TMLR publishes open access
% by default, consistent with the MIT-licensed dataset and pipeline.
% The TMLR style file `tmlr.sty' should be downloaded from
% https://jmlr.org/tmlr/author-guide.html and placed alongside this .tex
% file. After acceptance, replace the active \usepackage{tmlr} line below
% with \usepackage[accepted]{tmlr}.
%
% Fallback: if `tmlr.sty' is unavailable, comment the \usepackage{tmlr}
% line and uncomment the standalone-preprint block immediately after it
% to compile as a generic single-column preprint with natbib citations.
\usepackage{tmlr}
%
% --- Standalone preprint fallback (uncomment if tmlr.sty is unavailable):
% \usepackage[margin=1in]{geometry}
% \usepackage{times}
% \usepackage[round,sort]{natbib}
%
% ----- Historical NeurIPS submission notes (preserved for record) -----
% The original NeurIPS 2025 main-track preamble commentary is retained
% below for archival purposes; none of these directives are active under
% the current TMLR build. The corresponding NeurIPS 2026 style file is
% paper/neurips_2026.sty, which is used by the companion dataset paper
% (paper/dataset_paper.tex) for the NeurIPS 2026 Evaluations & Datasets
% Track submission.
%
...
% Note: page geometry is controlled by tmlr.sty under the active TMLR build;
% the \usepackage[margin=1in]{geometry} directive used by the prior NeurIPS
% layout is retained below as a comment for the standalone-preprint fallback.
% \usepackage[margin=1in]{geometry}
...
% =====================================================================
% TMLR's \author macro uses \name / \email / \addr fields and an optional
% \editor{} for the action editor. The block below conforms to TMLR's
% convention; the previous NeurIPS-style \author block is retained as a
% comment for the standalone-preprint fallback.
% Note. For the workshop paper template, both \title{} and \workshoptitle{}
% are required, with the former indicating the paper title shown in the
% title and the latter indicating the workshop title displayed in the
% footnote (NeurIPS workshop convention; not used under TMLR).
\title{Defeasible Reasoning as a Framework for Foundation Model Grounding, Novelty, and Belief Revision}

% The TMLR \author macro accepts any number of authors. Multiple authors
% are separated by \AND. Each author block contains \name (display name),
% \email (author email), and \addr (author affiliation address).
\author{\name Patrick Cooper \email patrick.cooper@colorado.edu \\
        \addr Department of Computer Science\\
              University of Colorado Boulder\\
              Boulder, CO 80309}

\editor{Submitted to TMLR}

% --- NeurIPS-style author block (preserved for the standalone-preprint
% fallback; activate by replacing the TMLR \author / \editor block above
% with this block when compiling without tmlr.sty):
%
% The \author macro works with any number of authors. There are two
% commands used to separate the names and addresses of multiple authors:
% \And and \AND. Using \And between authors leaves it to LaTeX to determine
% where to break the lines. Using \AND forces a line break at that point.
% So, if LaTeX puts 3 of 4 authors names on the first line, and the last
...
\subsection{DeFAb-Hard: Pre-Registered Difficulty Hardening}\label{subsec:defab_hard}

The 35 Level~3 instances of the Tier~0 evaluation occupy a narrow region of the difficulty spectrum. Empirical Level~3 statistics (Section~\ref{subsec:statistics}, regenerated April~2026): mean support size $|\Supp| = 6.7$ (max~10), mean predicate novelty $\Nov^* = 0.14$ (range $0$--$0.5$), single anomaly per instance, single superiority assertion. By construction, the current set rewards the Lakatosian \emph{monster-barring} strategy: a defeater that excludes the anomaly's class from the original rule's domain, reusing predicates already in $\Dm$. The IDP discovery that motivates the benchmark (Section~\ref{sec:intro}) demands the harder \emph{lemma-incorporation} strategy: introducing new predicates (\texttt{disordered}, \texttt{conf\_ensemble}, \texttt{func\_mech}) that distinguish the exception class. The current Tier~0 set is in this sense an \emph{easy subset}.

To honestly characterize the difficulty surface and to commit to evaluation regions where the belief revision deficit can be diagnosed rather than assumed, we pre-register a \textsc{DeFAb-Hard} subset, generated and evaluated as a single locked experiment. The pre-registration discipline is critical: the structural thresholds below are committed before any model is evaluated on the resulting instances, and the four-model panel will be re-evaluated as a unit, so the Hard subset cannot be retroactively narrowed to the cells where current models fail. This is the standard play (GSM1K~\cite{zhang2024gsm1k}, LiveCodeBench~\cite{jain2025livecodebench}) for adding adversarial difficulty without crossing into post-hoc selection.

\paragraph{Hardening axes.} \textsc{DeFAb-Hard} stratifies along four orthogonal structural axes, each mapped to a concrete generation procedure and an a priori threshold:

\begin{enumerate}[label=(H\arabic*),nosep,leftmargin=3em]
    \item \textbf{High predicate novelty.} Generate L3 instances whose gold defeater $r^*$ satisfies $\Nov(r^*, \Dm) \geq 0.5$, requiring at least half of $r^*$'s predicate symbols to be absent from $\pred(\Dm)$. The candidate language is expanded to include a controlled vocabulary $\mathcal{V}^+$ of structurally well-formed novel predicates with verified zero $n$-gram overlap on Common Crawl via \texttt{infini-gram}~\cite{liu2024infini}. Generation procedure: extend $\Dfull$ with the IDP-style construction of Example~\ref{ex:idp}, where the gold defeater introduces a new exception predicate plus a new mechanism predicate (Nov $\geq 2/4 = 0.5$ at minimum). Target instance count: 200 instances per source domain, balanced across biology, legal, materials, chemistry, and everyday commonsense (1{,}000 total). Actual count after generation: \textbf{-- (TBD)}.
    \item \textbf{Scaled theory size and support depth.} Generate at $|\D| \in \{50, 100, 200\}$ rules with support depth $\geq 5$ (i.e., the gold derivation chain traverses at least five rule applications between the facts and the target literal) and $\geq 3$ competing defaults whose superiority must be resolved as part of the construction. Procedure: extract dense subgraphs from Tier~1 cross-ontology theories that satisfy the depth and competing-default constraints; verify by the \texttt{rule\_validator} pipeline (Section~\ref{subsec:sources}). The defeater language $|\mathcal{L}_\D|$ grows polynomially with $|\D|$ but the relevant search space (paths through the dependency graph that reach the anomaly) grows exponentially in support depth, so a structural-depth gate is more discriminating than a raw rule-count gate. Target: 100 instances per $|\D|$ tier (300 total). Actual count after generation: \textbf{-- (TBD)}.
    \item \textbf{Multi-anomaly conservativity.} Generate L3 instances with $k \geq 3$ ground anomalies $\{\alpha_1, \ldots, \alpha_k\}$, only one of which (the target $\alpha_i$) is to be resolved; the gold defeater must satisfy the conservativity condition (Definition~\ref{def:conserv}) with respect to the remaining $k-1$ anomalies, leaving them defeasibly provable in the post-revision theory. Procedure: select $\Dfull$ theories that already contain multiple defeaters $\{r_1^*, \ldots, r_k^*\}$ for distinct anomalies; construct $\Dm$ by removing only $r_i^*$ and its superiority. The conservativity check, currently a default-pass on single-anomaly instances, becomes a non-trivial constraint that the model can fail by accidentally blocking other defeasible derivations. Target: 100 instances. Actual count after generation: \textbf{-- (TBD)}.
    \item \textbf{Synthetic contamination control completion.} The 409 structurally matched synthetic instances of Section~\ref{subsec:synthetic} are generated; only the four-model evaluation under formal modalities M2--M4 is pending. We commit to executing this evaluation as the highest-priority next experiment, with the pre-registered prediction below.
\end{enumerate}

\paragraph{Cross-axis intersection cell.} The four axes are designed to be combinable. The principal evaluation target is the \textsc{DeFAb-Hard-NovScaleMulti} intersection cell: instances satisfying H1, H2, and H3 jointly, requiring a high-novelty defeater that operates at depth $\geq 5$ in a $|\D| \in \{100, 200\}$ theory while preserving the derivability of $\geq 2$ unrelated anomalies. The intersection cell is the structurally hardest region of the benchmark; if any current model achieves non-trivial accuracy there, the underlying capability is meaningfully present. Cardinality is bounded above by the smallest individual-axis count, with target $\geq 50$ joint instances after de-duplication. Actual count: \textbf{-- (TBD)}.

\paragraph{Pre-registered hypotheses.} For each axis, the structural argument above predicts a specific direction and approximate magnitude of accuracy change relative to the easy 35-instance baseline. We commit to the following point predictions before any \textsc{DeFAb-Hard} or synthetic evaluation is executed:

\begin{itemize}[nosep]
    \item \textbf{H1 (high novelty).} Best-model L3 accuracy under CoT (currently DeepSeek-R1 92.9\% on the easy subset) drops by at least 40~pp on the high-novelty subset, because the model must \emph{construct} a predicate symbol from a controlled novel vocabulary rather than \emph{select} one from the theory's existing signature.
    \item \textbf{H2 (scaled depth).} Accuracy decreases monotonically in support depth: the gradient $\Delta\mathrm{Acc} / \Delta\,\text{depth}$ is negative for all four models, with reasoning-optimized models retaining their CoT advantage but exhibiting the same monotone decline.
    \item \textbf{H3 (multi-anomaly).} The conservativity error class (E4 in Section~\ref{subsec:errors}), currently $0.0\%$ across all models on the single-anomaly easy subset, rises to $\geq 25\%$ on multi-anomaly instances, becoming the dominant Level~3 error class and replacing decoder failure (E1) as the bottleneck.
    \item \textbf{H4 (contamination gap).} The contamination gap $\Delta_{\mathrm{synth}}$ at Level~3 is positive for all four models under M2--M4 modalities, with magnitude at least 15~pp for the reasoning-optimized models. The 92.9\% DeepSeek CoT figure is in this prediction substantially inflated by structural pattern memorization of canonical default--exception pairs.
    \item \textbf{H\textsubscript{HARD} (intersection cell).} On the \textsc{DeFAb-Hard-NovScaleMulti} intersection cell, rendering-robust Level~3 accuracy is at least 30~pp lower than on the easy 35-instance baseline for the strongest model under the strongest prompting strategy. A failure to find this gap (i.e., reasoning models continue to score above 60\% on the intersection cell) would constitute positive evidence that current frontier models possess robust defeasible-abduction capability and would necessitate revising the paper's central deficit claims. Pre-registration is committed at submission; thresholds (H1--H3) are fixed in code at \texttt{scripts/generate\_defab\_hard.py} (to be released with the final dataset); intersection-cell results are reported in Section~\ref{subsec:defab_hard_results} regardless of outcome.
\end{itemize}

If H1--H4 and H\textsubscript{HARD} are confirmed at the predicted magnitudes, the DeFAb-Hard subset constitutes the genuine difficulty regime over which the belief revision deficit can be measured, and the easy 35-instance subset is reframed as a calibration anchor. If they are rejected, the deficit is smaller than the framework predicts and the manuscript will be revised to reflect that finding. The objective is unambiguous: identify the difficulty boundary at which current frontier models reproducibly fail under the strongest available prompting strategy, then proceed to characterize whether that failure region collapses under the fine-tuning pipeline of Section~\ref{sec:finetuning} and the neural-guided tree search of Section~\ref{subsec:neural_search}.
...
Table~\ref{tab:contamination} reports the contamination gap $\Delta_{\mathrm{synth}}$ (Definition~\ref{def:contam_gap}) for all four models across structurally matched naturalistic and synthetic instance sets. The 409 structurally matched synthetic instances have been generated; model evaluation on the synthetic set is pending and values will be populated upon completion.
...
\subsection{DeFAb-Hard Results}\label{subsec:defab_hard_results}

The DeFAb-Hard subset (Section~\ref{subsec:defab_hard}) instantiates the four-axis hardening procedure for evaluating Level~3 belief revision in the difficulty regime where the Section~\ref{sec:intro} deficits are predicted to manifest. Generation, evaluation, and reporting follow the pre-registered protocol of Section~\ref{subsec:defab_hard}. Table~\ref{tab:defab_hard} reports rendering-robust Level~3 accuracy stratified by axis (H1 high-novelty, H2 scaled-depth, H3 multi-anomaly, H4 synthetic) and by the cross-axis intersection cell (H1 $\cap$ H2 $\cap$ H3); columns and table cells are placeholders pending generation and evaluation, and will be populated under the locked four-model panel of Section~\ref{subsec:evaluation} (DeepSeek-R1, GPT-5.2-chat, Claude~Sonnet~4.6, Kimi-K2.5).

\begin{table}[h]
\centering
\caption{DeFAb-Hard: rendering-robust Level~3 accuracy (\%) stratified by hardening axis and by the cross-axis intersection cell. Pre-registered targets are the H1--H4 and H\textsubscript{HARD} predictions of Section~\ref{subsec:defab_hard}; observed values are populated upon completion of the locked evaluation. The Easy column reproduces the existing 35-instance Tier~0 result for direct comparison. Cell convention: ``-- (TBD)'' marks values pending generation or evaluation; absolute decline (in pp) relative to the Easy column is the central reported quantity. Wilson 95\% CI half-widths and McNemar pairwise tests are reported alongside each populated cell.}
\label{tab:defab_hard}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Model} & \textbf{Easy} & \textbf{H1 Nov} & \textbf{H2 Scale} & \textbf{H3 Multi} & \textbf{H4 Syn} & \textbf{H1$\cap$H2$\cap$H3} \\
& (35) & ($\geq 0.5$) & ($|\D|{=}200$) & ($k{\geq}3$) & (matched) & (intersection) \\
\midrule
DeepSeek-R1 (CoT)            & 92.9\% & -- & -- & -- & -- & -- \\
GPT-5.2-chat (CoT)           & 87.1\% & -- & -- & -- & -- & -- \\
Claude Sonnet~4.6 (Direct)   & 23.6\% & -- & -- & -- & -- & -- \\
Kimi-K2.5 (CoT)              & 27.6\% & -- & -- & -- & -- & -- \\
\midrule
Pre-registered prediction (best model) & --- & $\leq 53\%$ & monotone $\downarrow$ & --- & $\leq 78\%$ & $\leq 63\%$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Reporting commitments.} For each populated cell we report (i)~point accuracy with Wilson 95\% CI half-width, (ii)~per-model McNemar test against the Easy baseline, (iii)~the Section~\ref{subsec:errors} error-class distribution (E1--E5) with explicit attention to E4 (conservativity violations) on the H3 and intersection columns, (iv)~the graded-score distribution of Section~\ref{subsec:partial_credit}, and (v)~for the H4 column, the per-model contamination gap $\Delta_{\mathrm{synth}}(M, 3)$ with bootstrap confidence interval over the 409 matched instance pairs.

\paragraph{Outcome interpretation.} If the predicted gaps materialize at the magnitudes committed in Section~\ref{subsec:defab_hard} (H1: $\geq 40$~pp drop; H3: E4 share $\geq 25\%$; H4: $\Delta_{\mathrm{synth}} \geq 15$~pp; H\textsubscript{HARD}: intersection-cell drop $\geq 30$~pp), the result establishes that the easy 35-instance ceiling reflects difficulty under-sampling rather than capability and shifts the headline of the paper to the intersection-cell numbers. If the gaps are smaller than predicted (or absent), the manuscript will be revised to reflect that finding and the central deficit claims will be tempered or withdrawn. The objective is to identify the difficulty boundary at which current frontier models reproducibly fail under the strongest available prompting strategy, and only then to evaluate whether the fine-tuning pipeline of Section~\ref{sec:finetuning} or the neural-guided tree search of Section~\ref{subsec:neural_search} closes that gap.
...
\subsubsection{Two-Principle Search-Space Reduction}\label{subsubsec:two-principle}
...
\subsubsection{The AlphaZero Loss for Defeasible Theories}\label{subsubsec:agz-loss}
...
Adapting to defeasible theory construction, $z$ becomes the verifier-backed terminal score from a self-played conflict resolution game (Definition~\ref{def:conflict_game}), normalized to $[-1, +1]$:
\[
...
\subsubsection{Policy Iteration Framing of Expert Iteration}\label{subsubsec:policy-iter}

The expert iteration loop above can be made formally precise as classical approximate policy iteration~\cite{silver2017alphagozero}. Two operators alternate. \emph{Policy improvement}: MCTS, when run to convergence with the current network as prior and value, produces a search policy $\pi = \alpha_\theta(s)$ that is provably stronger than the raw network policy $p_\theta(s)$ because it incorporates lookahead. The proof is the standard policy-improvement theorem applied to the search-derived $Q$-values. \emph{Policy evaluation}: self-play games against the current best player produce trajectories $(s_t, \pi_t, z_t)$ whose terminal outcomes $z_t$ are samples of the value function under the improved policy. The training step projects $(\pi, z)$ back into the network's function space by gradient descent on the AlphaZero loss above (Section~\ref{subsubsec:agz-loss}).
...
\subsubsection{Diversity vs.\ Sharpness in the Policy Prior}\label{subsubsec:diversity-prior}
...
The same trade-off is visible in our pipeline (Section~\ref{sec:finetuning}). The SFT-trained model (Section~\ref{subsec:sft}) is broad: trained on diverse gold hypotheses, it spreads probability mass across many plausible defeaters per conflict. The DPO-tuned model (Section~\ref{subsec:dpo}) is sharper: trained on preference pairs, it concentrates probability on the highest-scoring defeater observed in training. The GRPO-tuned model (Section~\ref{subsec:rlvr}) is sharpest. The AlphaGo lesson predicts that the SFT model gives the better MCTS prior despite scoring lower in direct evaluation, because conflicts in the Lakatos-correspondence sense (Proposition~\ref{prop:lakatos}) typically admit multiple valid resolutions at varying conservativity levels, and the search needs to consider all of them. A concrete experimental design: hold the search algorithm and value head fixed, vary only the policy prior across $\{p_{\mathrm{SFT}}, p_{\mathrm{DPO}}, p_{\mathrm{GRPO}}\}$, and report MCTS-derived composite-score as a function of search budget.

\subsubsection{MCTS Engineering: Virtual Loss, Dirichlet Noise, Temperature, Resignation}\label{subsubsec:mcts-engineering}
...
\subsubsection{Asynchronous APV-MCTS Architecture for Defeasible Search}\label{subsubsec:async-apvmcts}
...
\subsubsection{Domain Knowledge for Defeasible AlphaZero}\label{subsubsec:domain-knowledge}
...
\begin{enumerate}[nosep]
    \item \textbf{Defeasible derivation rules.} The Antoniou--Billington--Governatori conditions for $+\partial q$ (Definition~\ref{def:defderiv}) are provided as the transition function: given $(\D_t, a_t)$, the verifier deterministically produces $\D_{t+1} = \D_t \cup \{a_t\}$ and the updated anomaly set $\mathcal{A}_{t+1}$.
    \item \textbf{Verifier scoring function.} The scoring function $\Score(d, \D, q)$ of Section~\ref{subsec:partial_credit} is provided as the terminal reward; the network does not approximate it.
...
\subsubsection{Tabula Rasa Feasibility: Quantitative Grounding}\label{subsubsec:tabula-rasa}
...
Whether the same result transfers to defeasible theory construction depends on the relationship between the policy network's effective branching factor (after pruning the $\sim 10^{12}$-element defeater language $\mathcal{L}_\D$, Section~\ref{subsubsec:two-principle}) and the density of valid defeaters in that space. In Go, the random-initialization policy still concentrates probability on \emph{some} legal moves, and self-play exploration with Dirichlet noise eventually finds informative gradients. In defeasible construction, a uniform prior over $\mathcal{L}_\D$ assigns vanishing probability to any specific valid defeater, and the gradient signal from random rollouts may be too sparse to bootstrap. The branching-factor disparity (Go $b \approx 250$, defeasible $b \sim 10^{12}$) suggests that aggressive prior shaping via SFT warm-start (Section~\ref{subsec:sft}) likely remains rational engineering even if tabula-rasa is theoretically feasible. A concrete empirical test: ablate the SFT phase on a small Tier-0-derived training run, hold DPO and GRPO fixed, and compare self-play composite-score trajectories at 24h, 48h, and 72h. If the random-init trajectory catches up, tabula-rasa is feasible; if it stalls, the SFT prior is doing irreplaceable work.

\subsubsection{Dual-Head vs.\ Separate Networks: Architecture Ablation Plan}\label{subsubsec:arch-ablation}
...
\paragraph{Belief revision capability is latent, not absent (on the easy subset).} The central finding on the existing 35-instance Tier~0 Level~3 set is more nuanced than an outright failure: belief revision capability is present in reasoning-optimized models (DeepSeek-R1 65\%, GPT-5.2 48\% under CoT) but is highly sensitive to prompt structure. This reframes the research question: the challenge is not to instill a completely absent capability but to understand when and why it surfaces. The Orient Express analogy of Section~1 requires revision: today's detectives can, under the right conditions, override the single-perpetrator default with a conservative solution---but only when explicitly scaffolded through the logical steps of that override. Without the scaffold, reasoning-model performance collapses to near-random (direct L3: DeepSeek-R1 37\%, GPT-5.2 8\%). The scaffold-dependence itself is informative: the capability is not robustly internalized.

This nuance is, however, conditional on the Tier~0 Level~3 difficulty distribution---35 expert-curated instances with mean predicate novelty $\Nov^* = 0.14$ (range $[0, 0.5]$), mean support size 6.7 (max 10), single anomaly per instance, and unmeasured contamination gap $\Delta_{\mathrm{synth}}$. Section~\ref{subsec:defab_hard} pre-registers DeFAb-Hard, a structurally hardened evaluation along four axes (high-novelty $\Nov \geq 0.5$, $|\D| \in \{50, 100, 200\}$ with support depth $\geq 5$ and $\geq 3$ competing defaults, $k \geq 3$ multi-anomaly conservativity, and the pending 409-instance synthetic evaluation). Five point predictions (H1, H2, H3, H4, H\textsubscript{HARD}) are committed at submission time. Until those numbers are populated, the ``latent but recoverable under CoT'' framing should be read as conditional on the easy-subset distribution; the central question of whether reasoning-optimized models possess a robust defeasible-abduction capability or have merely interpolated the canonical default--exception pairs of textbook examples is decided by the intersection-cell H\textsubscript{HARD} number in Table~\ref{tab:defab_hard} (Section~\ref{subsec:defab_hard_results}). The objective of the DeFAb-Hard construction is to identify the difficulty boundary at which current frontier models reproducibly fail under the strongest available prompting strategy, then to characterize whether that failure region collapses under the fine-tuning pipeline of Section~\ref{sec:finetuning} or the neural-guided tree search of Section~\ref{subsec:neural_search}.
...
\paragraph{Live StarCraft~II grounding (sc2live).}\label{subsec:sc2live}
The hand-authored \texttt{rts\_engagement} KB establishes the formal ROE schema, but a long-standing concern about benchmark-only ROE evaluation is that hand-crafted scenarios may not capture the situation-recognition burden of live play. To close this gap, we expose the polynomial-time verifier to live game state via the \texttt{src/blanc/sc2live/} bridge (BurnySC2 / python-sc2). Three external surfaces compose the bridge. \textsc{ObservationLifter} converts each \texttt{BotAI.state} tick into a defeasible \texttt{Theory}: own and enemy unit positions, health, ammunition, line-of-sight, exclusion-zone membership, and worker/military classification all become ground facts under the same predicate signature used by \texttt{rts\_engagement}, so the strict, defeasible, and defeater rules of the hand-authored ROE theory remain valid without modification. \textsc{ActionCompiler} translates derived literals (e.g., \texttt{authorized\_to\_engage(unit, target)}, \texttt{ordered\_to\_retreat(unit)}) into asynchronous \texttt{BotAI} unit orders, providing the action interface for closed-loop policies. \textsc{ReplayTraceExtractor} ingests \texttt{.SC2Replay} files and yields a sequence of $(\text{theory state}, \text{observed action})$ pairs, enabling instance generation directly from real games (and, for the LLM tier, supervised data without simulator runtime). The \texttt{DeFAbBot} dispatches per-tick on \texttt{propose\_orders} (the \texttt{CommanderPolicy} of Section~\ref{subsec:roe-compliance}) versus \texttt{propose\_defeaters} (the \texttt{ScriptedPolicy} and \texttt{LLMPolicy} of Section~\ref{subsec:agents}), with the polynomial-time \textsc{DefeasibleEngine} as the unchanged fixed point that scores every order against the same conservativity check applied at Levels~1--3 (Section~\ref{subsec:partial_credit}). Each session produces a per-game theory of $\sim$200 rules and yields several thousand Level~1 fact-completion and Level~2 rule-abduction instances; the headless Linux setup is documented in \texttt{scripts/install\_sc2\_linux\_headless.sh} and the SLURM launch in \texttt{hpc/slurm\_sc2\_grounding.sh}. The \texttt{sc2live} optional dependency group (\texttt{pip install blanc[sc2live]}) keeps \texttt{burnysc2} out of the core requirements, and 67 unit tests in \texttt{tests/sc2live/} cover the bridge with no SC2 binary required, with an additional \texttt{sc2\_live} pytest marker for live-binary integration tests.

\paragraph{Lux AI S3 as a parallel game-grounded KB.}
To distinguish the structural defeasible-reasoning capability under test from any vocabulary-specific affinity for StarCraft~II, we provide a second game-grounded KB. The Lux~AI~Season~3 (NeurIPS~2024) competition rules are encoded as a defeasible theory in \texttt{scripts/generate\_lux\_instances.py}: $\sim$130 rules, $\sim$60 facts, $\sim$80 evaluation instances spanning resource collection, robot programming, and adversarial map control. Lux shares the formal schema (strict, defeasible, defeater, superiority) with both \texttt{rts\_engagement} and \texttt{sc2live} but has zero predicate or constant overlap with biology, legal, materials, or StarCraft. This makes it a controlled second testbed for the cross-environment transfer experiment of Section~\ref{subsec:cross-env-transfer}: any positive transfer between Lux and the other RTS-class or knowledge-base domains must be attributable to the shared defeasible structure rather than to lexical co-occurrence. The Lux extractor reuses the same instance generation pipeline and decoder cascade as the cross-ontology and biomedical tiers; no game-specific decoder logic is introduced.
...
\paragraph{ROE compliance experiment with verifier-gated commander policy.}\label{subsec:roe-compliance}
Experiments~1--3 evaluate models on pre-generated abductive instances. A complementary experiment evaluates LLM-as-commander policies in closed loop, where the verifier acts as the ground-truth ROE judge each macro-tick. The \texttt{CommanderPolicy} (\texttt{src/blanc/sc2live/policies/commander.py}) receives a structured situation report from \texttt{build\_situation\_report} (\texttt{src/blanc/sc2live/situation\_report.py})---ground facts about own and enemy units, active mission constraints, exclusion-zone membership, and currently triggered defeaters---and emits a list of high-level battlefield orders parsed by \texttt{parse\_orders}. Each order is scored by \texttt{check\_order} against the \texttt{rts\_engagement} defeasible theory, returning a \texttt{ComplianceVerdict} that records whether the order is permitted under the active ROE. Three enforcement modes isolate the role of verification:

\begin{itemize}[nosep]
    \item \textbf{B0 (trust-LLM, post-hoc logging):} orders are applied verbatim; compliance is logged but never blocked. This measures the LLM's intrinsic ROE compliance rate without verifier intervention.
    \item \textbf{B1 (audit-only):} orders are applied verbatim and every order is scored by the verifier with results streamed to the trace, but no order is blocked. This isolates the cost of running the verifier from the cost of acting on its verdict.
    \item \textbf{B2 (verifier-gated re-prompt):} non-compliant orders are rejected and the LLM is re-prompted with the verdict appended to the situation report (up to $K$ re-prompts per tick). This is the strongest hybrid configuration, in which the verifier is in the loop at decision time.
\end{itemize}

Three formal hypotheses guide the analysis (computed by \texttt{experiments/roe\_compliance\_analysis.py}):
...
Both \emph{quiz} and \emph{live} backends are supported by \texttt{scripts/run\_roe\_compliance\_experiment.py}: the quiz backend evaluates against curated scenarios with gold-answer compliance verdicts (an in-distribution unit test of the policy), while the live backend runs the \texttt{DeFAbBot} against the SC2 binary and emits a per-tick \texttt{.jsonl} trace under \texttt{data/roe\_compliance/}. A regression suite of 79 tests across \texttt{tests/sc2live/} (covering \texttt{orders\_schema}, \texttt{compliance}, \texttt{situation\_report}, \texttt{commander\_policy}) and \texttt{tests/integration/test\_roe\_compliance\_quiz.py} pin the policy interface, the parser tolerance to LLM whitespace and case variation, the verdict semantics, and the dispatch logic in \texttt{DeFAbBot.on\_step} that routes between \texttt{propose\_orders} (CommanderPolicy) and \texttt{propose\_defeaters} (ScriptedPolicy / LLMPolicy).

\paragraph{Experiment~2 results: RTS-engagement Level~3 evaluation.}\label{subsec:rts-eval-results}
We executed Experiment~2 on the certified \texttt{rts\_engagement} KB (April 13, 2026) using the same four-model panel as Section~\ref{subsec:revision_results}---DeepSeek-R1, GPT-5.2-chat, Claude Sonnet~4.6, Kimi-K2.5---under the M4 (pure formal) modality and both direct and chain-of-thought prompting strategies. The evaluation set is the six hand-crafted Level~3 seed conflicts of Section~\ref{subsec:sources} (\texttt{rts-l3-0} through \texttt{rts-l3-5}), each evaluated under both prompting strategies for a total of 12 evaluations per model. Table~\ref{tab:rts_eval} reports the headline accuracies; raw per-instance traces are committed under \texttt{experiments/results/rts\_foundry-\{deepseek,gpt,claude,kimi\}\_20260413\_*.json}.

\begin{table}[h]
\centering
\caption{RTS-engagement Level~3 evaluation: accuracy on six L3 seed conflicts, M4 pure formal modality, direct and CoT pooled (12 evaluations per model). Wilson 95\% CI half-width is $\pm 25$~pp at $n = 12$, so per-model figures are descriptive. Error column reports the dominant non-correct error class. Aggregate cost across all four model sweeps was \$0.67.}
\label{tab:rts_eval}
\small
\begin{tabular}{@{}lccl@{}}
\toprule
...
Three observations stand out. First, GPT-5.2-chat leads with 75\% accuracy, distinguishing the RTS-engagement domain from biology/legal/materials (where DeepSeek-R1 dominates Level~3 under CoT): the formal regularity of the ROE rule syntax appears to favor the instruction-following profile that GPT-5.2 exhibits in direct mode, partially confirming hypothesis (H4) that ROE syntax produces fewer E1 errors than the more heterogeneous knowledge-base domains. Second, Claude Sonnet~4.6 again exhibits the architectural dissociation reported in Section~\ref{subsec:cot_results}: 5 of 6 direct-prompt evaluations succeed (with three of them scoring full credit on conservatively constructed defeaters) while every CoT evaluation fails on E2 derivation failure. This replicates the instruction-versus-reasoning model split documented in the main Tier~0 evaluation, on a fully disjoint domain whose vocabulary (military units, exclusion zones, mission codes) shares no surface overlap with the LKIF Core legal training distribution. Third, Kimi-K2.5's failure profile is dominated by E1 decoder failures (9 of 12 evaluations), again replicating the pattern reported on Tier~0 Level~3 (Section~\ref{subsec:revision_results}). The Wilson CI half-width of $\pm 25$~pp at $n = 12$ precludes per-model significance claims, but the qualitative agreement with the Tier~0 cross-model rank ordering on Claude and Kimi is consistent with the central thesis: the measured capability gap is structural rather than vocabulary-bound.

\paragraph{Cross-environment transfer (E5).}\label{subsec:cross-env-transfer}
The five-domain panel (biology, legal, materials, RTS-engagement, Lux AI S3) plus the dynamic \texttt{sc2live} bridge enables a sixth experimental condition designed to test the central claim that DeFAb measures structural defeasible reasoning rather than vocabulary-bound knowledge retrieval. The script \texttt{experiments/cross\_env\_transfer.py} maintains a registry of six environments---\texttt{sc2live}, \texttt{rts\_engagement}, \texttt{lux\_ai\_s3}, \texttt{biology}, \texttt{legal}, \texttt{materials}---and evaluates a model under one configuration across all six in a single run, producing a row-per-environment Level~2 / Level~3 / graded-score table. Hypothesis~H5 (cross-environment transfer):

\begin{quote}
If models fine-tuned on one domain (e.g., \texttt{sc2live} via the GRPO loop of Section~\ref{subsec:rlvr}) score comparably on Level~3 instances drawn from the other five environments, this constitutes positive evidence that the fine-tuning pipeline imparts a domain-general defeasible-reasoning capability rather than environment-specific pattern matching.
\end{quote}

The H5 evaluation is structurally orthogonal to the per-environment evaluations of Sections~\ref{subsec:revision_results} and~\ref{subsec:rts-eval-results}: it varies the \emph{environment} axis with the \emph{model} held fixed, while the prior evaluations vary the model with the environment held fixed. The script supports any \texttt{ModelInterface} provider (foundry, CURC vLLM, mock) and accepts a \texttt{--model} argument that points to a fine-tuned checkpoint, enabling direct comparison between base-model and post-DPO/post-GRPO transfer profiles. Initial mock-provider runs validate the harness; foundry and CURC sweeps are executing in batches of 20 instances per environment, with results streamed to \texttt{experiments/results/cross\_env\_transfer\_\{provider\}\_\{timestamp\}.json}.
...
At Level~3 (defeater abduction), the picture on the existing 35-instance Tier~0 set is more structured than a simple failure. Reasoning-optimized models (DeepSeek-R1 65\%, GPT-5.2 48\%) demonstrate substantial belief revision capability under a generation-appropriate chain-of-thought scaffold, while instruction-optimized models (Claude 16\%, Kimi 14\%) perform better under direct prompting. The CoT-sensitivity of reasoning-model performance---a 56--79 percentage-point swing from direct to CoT prompting---establishes that belief revision capability is \emph{latent} in these models rather than absent, but requires explicit prompting structure to surface reliably. The architectural dissociation (CoT helps reasoning models, hurts instruction models) is statistically confirmed and constitutes a distinct finding about how model families respond to different task formulations.

This nuanced reading is conditional on the easy-subset difficulty distribution. The 35 instances have mean predicate novelty 0.14, mean support size 6.7, single-anomaly conservativity (a default-pass), and an unmeasured contamination gap $\Delta_{\mathrm{synth}}$. Section~\ref{subsec:defab_hard} pre-registers the DeFAb-Hard subset along four orthogonal axes (high novelty, scaled depth, multi-anomaly conservativity, synthetic contamination) with five point predictions H1--H4 and H\textsubscript{HARD} committed at submission. Whether the latent-but-recoverable framing survives DeFAb-Hard---in particular whether reasoning-optimized models retain non-trivial accuracy on the H1 $\cap$ H2 $\cap$ H3 intersection cell---is the headline empirical question this manuscript will answer in its next revision. The objective is to identify the difficulty boundary at which current frontier models reproducibly fail under the strongest available prompting strategy, and only then to test whether the fine-tuning pipeline of Section~\ref{sec:finetuning} or the neural-guided tree search of Section~\ref{subsec:neural_search} closes that gap.
