...
The base evaluation uses 409 expert-curated Tier~0 instances (35 Level~3); Wilson 95\% CI half-widths are $\pm 15$--$17$ pp on aggregate Level~3 accuracy. The M1 decoder operates approximately by design. Automated Level~3 generation from Tier~1 remains limited by sparse ConceptNet \texttt{NotCapableOf} edges (329 total English edges); the richest automated defeater sources are Gene Ontology \texttt{NOT}-qualifiers (1,250) and Wikidata P2303 (11,557). The synthetic contamination control mitigates but does not eliminate the confound: structural patterns of defeasible theories may themselves be memorized.
...
\item {\bf Claims}
    ...
    \item[] Justification: The introduction lists four numbered contributions, each developed in corresponding sections. All accuracy figures include Wilson 95\% CIs. Claims about dataset scale are backed by Table~\ref{tab:kbsummary}.
