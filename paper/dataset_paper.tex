...
% TODO: Update statistics once all tiers are finalized.
% TODO: Confirm final instance counts match codebase.
\end{abstract}
...
\begin{enumerate}[label=(\arabic*),nosep]
    ...
    \item \textbf{A cross-ontology knowledge base extraction methodology} for generating defeasible rule bases at scale from heterogeneous publicly funded knowledge sources. The procedure pairs taxonomic hierarchies with behavioral property graphs to synthesize defeasible rules; exception relations serve as structured defeater sources, enabling automatic instance generation without manual authoring. The dataset spans 14 knowledge sources and 4.28 million materialized rules.
...
Given a converted theory $\D$ and a derivable target $q$, we generate evaluation instances by exploiting the grounding structure. An element $e$ is full-theory critical if $\D \setminus \{e\} \not\pPartial q$; this is decidable in polynomial time. We ablate a critical element to form $\Dm = \D \setminus \{e\}$, inject distractors, and define the gold set $\Hgold = \{h \mid \Dm \cup \{h\} \pPartial q,\; h \text{ minimal}\}$. Instances are stratified:
...
A rendering codec $\rho = (\rho_{\mathrm{enc}}, \rho_{\mathrm{dec}})$ translates between formal theories and natural language. We define four encoding modalities: narrative (M1), semi-formal (M2), annotated formal (M3), and pure formal (M4). The decoder maps model outputs back to formal rules via exact match (D1), template extraction (D2), or semantic parsing (D3), with round-trip consistency validated prior to evaluation.

The headline metric is \textit{rendering-robust accuracy}: the worst-case accuracy over all four modalities, ensuring scores reflect reasoning about the underlying epistemic structure rather than surface-form sensitivity.
...
% TODO [CRITICAL]: This section currently describes planned analyses rather
% than presenting results. Before submission, the following must be computed
% and presented as concrete tables/figures:
%
% 1. Instance counts per level, per source KB, per partition function
%    (joint distribution to assess balance).
%
% 2. Structural difficulty distributions: marginal and joint distributions
%    of the difficulty tuple components (level, |Supp|, |H_gold|, min|h|, Nov*).
%    Test independence via mutual information.
%
% 3. Novelty and revision spectrum (Level 3): joint distribution of Nov
%    and d_rev, cross-tabulated against resolution strength.
%
% 4. Yield curves Y(kappa_rand(delta), Q) as a function of delta for each
%    source KB, with parametric model fits.
%
% 5. Partition sensitivity: two-sample tests on difficulty distributions
%    across the five partition families.
%
% 6. Defeater quality-volume tradeoff (Level 3): precision-volume Pareto
%    frontier over ConceptNet weight threshold and Wikidata P2303 count.

\paragraph{Volume and balance.}
Table~\ref{tab:kbsummary} summarizes total extraction results. The Tier~0 evaluation set contains 374 Level~1/2 instances and 35 Level~3 instances across three domains (biology, legal, materials). Tier~1 produces 324,511 instances (182,652 Level~1, 141,859 Level~2) across five domains.

% TODO: Add a detailed breakdown table by level x domain x partition.
% TODO: Report Level 3 instance counts from automated defeater extraction
%       (ConceptNet NotCapableOf: 329 English edges; GO NOT-qualifiers: 1,250;
%       Wikidata P2303: 11,555). These are the richest automated Level 3 sources.

\paragraph{Structural difficulty.}
Instance difficulty is characterized by a structural tuple $\sigma(I) = (\ell, |\Supp|, |\Hgold|, \min|h|, \Nov^*)$ combining level, support set count, gold set size, hypothesis complexity, and minimum novelty.

% TODO: Compute and present the actual distributions. Generate figures
%       showing marginal histograms and joint scatter plots.

\paragraph{Decoder validation.}
Validation over all 409 gold-standard hypotheses confirms that modalities M2, M3, and M4 achieve 100\% round-trip recovery. M1 (narrative) operates approximately by design.
...
% TODO: Add Qwen 2.5-72B and Qwen 2.5-32B results for scaling analysis.
% TODO: Add open-source model evaluations from CURC Alpine (vLLM).
...
Each instance is presented under all four rendering modalities (M1--M4) and two prompting strategies (direct and chain-of-thought). The CoT scaffold mirrors defeasible derivation structure: identify applicable rules, determine missing/blocking elements, check for attackers and superiority, and propose a conservative hypothesis.
...
All models drop to 14--22\% on narrative rendering (M1) versus 73--94\% on formal modalities (M2--M4). This 55--70 pp gap is the largest effect in the evaluation, larger than any inter-model or inter-level difference. Rendering-robust accuracy is dominated by M1 performance.
...
The DeFAb dataset, generation pipeline, extraction infrastructure for 14 knowledge sources, and evaluation harness are released under the MIT license.
...
% Full proofs are available in the supplementary materials. See the companion
% technical report [Cooper, 2026] for complete proof details.
% TODO: Decide whether to include full proofs inline or reference the
%        companion paper.
...
\item \textbf{Rule base:} 4.28 million materialized rules from 14 knowledge sources (Tiers 0--3), plus 29.5 million UMLS rules.
...
\item {\bf Theory assumptions and proofs}
    \item[] Answer: \answerYes{}
    \item[] Justification: All propositions and theorems are numbered and proved in Appendix~\ref{app:proofs}. Assumptions are stated explicitly (e.g., Proposition~\ref{prop:strict} assumes all-strict partition).
...
\item {\bf Experimental setting/details}
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{subsec:setup} specifies all models, API endpoints, decoding parameters (greedy, temperature~$=0$), prompting strategies, distractor count ($k=5$), and the three-stage decoder pipeline.
