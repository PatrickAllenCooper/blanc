...
The M5 modality (Section~\ref{subsec:m5}) tests whether defeasible reasoning transfers to multimodal settings. Research on visual generics~\cite{park2025visage} shows VLMs fail on atypical instances---precisely the entities governed by defeater rules. Whether M5-replace accuracy tracks M1 narrative degradation or diverges from it distinguishes a shared grounding bottleneck from dissociable visual and linguistic deficits.
...
\paragraph{Hosting.}
The dataset is hosted on HuggingFace Datasets at \url{https://huggingface.co/datasets/PatrickAllenCooper/DeFAb} with Croissant metadata (core and RAI fields). The generation pipeline and evaluation harness are released on GitHub. All source KB licenses are enumerated in the Datasheet (Appendix~\ref{app:datasheet}); derived DeFAb instances and code are MIT-licensed. The dataset will be maintained for at least 5 years; Tier~0 instances are frozen for cross-study comparability.
