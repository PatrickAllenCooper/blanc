...
\begin{table}[h]
\centering
\caption{\textsc{DeFAb-Hard} pilot statistics. The symbolic solver achieves 100\% on all three axes; LLM evaluation on the open-source baselines (Qwen-72B/32B, DeepSeek-R1-Distill-70B) is queued on CURC (Section~\ref{subsec:ft_setup}). Frontier-model evaluation via the Foundry API is in progress concurrently with paper preparation; final results will be incorporated upon completion.}
\label{tab:defabhard}
\small
