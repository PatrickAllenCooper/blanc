...
\begin{tabular}{@{}llrrl@{}}
\toprule
Tier & Primary Sources & Est.\ Rules & Est.\ Instances & Timeline \\
\midrule
0 (baseline) & YAGO, WordNet, LKIF, MatOnto & 2,318 & 374 L1/L2 + 35 L3 & Done \\
1 (this paper) & OpenCyc+ConceptNet, YAGO full & 100K--350K & 33K--90K & Weeks 8--14 \\
2 (biomedical) & UMLS, SNOMED CT, GO, MeSH & +850K--3M & +130K--480K & 2--6 mo. \\
3 (encyclopedic) & Wikidata+P2303, DBpedia, BabelNet & +3M--15M & +480K--2.4M & 6--12 mo. \\
4--5 (web-scale) & NELL, Freebase, SUMO, FrameNet & +6M--25M & +960K--4M & 1--2+ yr. \\
\bottomrule
\end{tabular}
...
DeFAb provides the formal infrastructure --- polynomial-time instance generation, verifier-backed gold hypotheses, explicit novelty and revision metrics, and a rendering-robust protocol --- to track progress on the belief revision deficit as training paradigms evolve.
...
\item {\bf Experimental setting/details}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: No training is performed; DeFAb is a zero-shot evaluation benchmark. Section~\ref{sec:experiments} specifies all relevant details: model names and API versions, decoding parameters (greedy, temperature~$=0$, \texttt{max\_tokens}), prompting strategy structure, $k=5$ distractor count, partition function families, and the three-stage decoder pipeline with validation results.
    \item[] Guidelines:
    \begin{itemize}
        \item The answer NA means that the paper does not include experiments.
