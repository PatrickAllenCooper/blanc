...
The synthetic contamination control reproduces the same panel on the 35 structurally matched L3 synthetic instances at M2 (semi-formal) and M4 (pure formal), with both prompting strategies (560 calls). Averaging across the two synthetic modalities, per-model L3 accuracy drops uniformly: DeepSeek-R1 65.0\%~$\to$~52.9\% ($\Delta_{\mathrm{synth}} = +12.1$~pp), GPT-5.2-chat 47.5\%~$\to$~30.9\% ($+16.6$~pp), Claude~Sonnet~4.6 16.4\%~$\to$~2.1\% ($+14.3$~pp), Kimi-K2.5 14.2\%~$\to$~2.9\% ($+11.3$~pp); mean $+13.6$~pp. M2 and M4 give the same directional result, ruling out a modality-specific artifact. The uniformly positive sign is consistent with partial vocabulary-level memorization on naturalistic L3, but the cognitive load of unfamiliar predicates may also contribute. Even after gap correction the best-model L3 accuracy (52.9\%) remains far below the symbolic ceiling, so the structural-deficit conclusion is robust to the control.
