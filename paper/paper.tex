...
Evaluation of six frontier models reveals a sharp discontinuity: models achieve $\geq 88\%$ accuracy on Level~2 rule abduction under direct prompting, demonstrating strong formal grounding capability, yet collapse to $\leq 2\%$ on Level~3 defeater abduction, confirming the belief revision deficit as a distinct and persistent failure mode. Chain-of-thought prompting \emph{reduces} Level~2 accuracy by $15$--$20$ percentage points across models, an ``overthinking'' effect specific to formal candidate-selection tasks. These findings suggest that the grounding and revision capabilities are architecturally separable, with current training paradigms solving the former but leaving the latter essentially unaddressed.
\end{abstract}
...
\textbf{[Table 1: Per-model rendering-robust accuracy by level goes here.]}

\textbf{[Table 2: Per-modality accuracy breakdown (M1--M4) per model and level goes here.]}

\subsection{Grounding Results (Level~2)}\label{subsec:grounding_results}

\textbf{[Section to be written from full CURC evaluation results.]}

Preliminary pilot results (20 instances/domain, M4$+$M2, direct$+$CoT prompting) establish a baseline: under direct prompting, GPT-5.2-chat achieves 98.3\% Level~2 accuracy and Claude~Sonnet~4.6 achieves 99.2\%. Rendering-robust accuracy (worst-case over modalities) is 47.5\% and 55.0\% respectively, indicating significant variation across modalities that is masked by per-modality averages. Claude~Sonnet~4.6 outperforms GPT-5.2-chat on this benchmark despite the latter being marketed as a reasoning-optimized model, a pattern that recurs across domains and modalities. Domain breakdown shows consistency: both models achieve $>85\%$ accuracy across biology, legal, and materials domains under direct prompting, validating that the benchmark's grounding component is domain-agnostic in difficulty at the pilot scale.

\subsection{Belief Revision Results (Level~3)}\label{subsec:revision_results}

\textbf{[Section to be written from full CURC evaluation results.]}

Both closed-source models evaluated in the pilot collapse at Level~3: GPT-5.2-chat achieves 1.4\% accuracy and Claude~Sonnet~4.6 achieves 2.1\%, each correctly identifying the conservative defeater on only 1--3 of 35 instances across two modalities. Error taxonomy reveals qualitatively different failure signatures: GPT-5.2 errors are dominated by E1 (decoder failure, 55\%), indicating the model produces responses in formats incompatible with the formal hypothesis representation; Claude errors are dominated by E2 (derivation failure, 64\%), indicating successful response formatting but systematic failure to identify the defeasible gap. This pattern suggests that instruction optimization improves output formatting for formal tasks while reasoning optimization does not, and that neither eliminates the fundamental belief revision deficit.

Level~3 graded scores confirm the pattern: mean graded score is 0.014 (GPT-5.2) and 0.021 (Claude), both well below 0.25 (the first partial-credit tier corresponding to syntactically valid but non-resolutive hypotheses).

\subsection{Chain-of-Thought Analysis}\label{subsec:cot_results}

\textbf{[Section to be written from full CURC evaluation results.]}

Pilot results quantify the CoT degradation at Level~2: $\Delta_{\mathrm{CoT}} = -20.0$ pp (GPT-5.2) and $-15.0$ pp (Claude). The degradation is specific to the formal-candidate selection setting: under direct prompting, both models output a candidate rule verbatim with near-perfect accuracy, while under CoT prompting they produce extended reasoning chains whose final lines are less likely to exactly match any candidate string. This confirms the ``overthinking'' interpretation. At Level~3, $\Delta_{\mathrm{CoT}} \approx 0$ for both models, confirming that the Level~3 failure is not a prompting artifact.

\subsection{Scaling Analysis}\label{subsec:scaling_results}

\textbf{[Section to be written from full CURC evaluation results.]}

\subsection{Symbolic Ceiling}\label{subsec:symbolic_results}

\textbf{[Section to be written after \texttt{clingo} baseline is run.]}


\section{Discussion}\label{sec:discussion}

\textbf{[Section to be written from full evaluation results.]}

The pilot findings motivate three claims that full evaluation will substantiate or refute. \emph{First}, the belief revision deficit is distinct and not reducible to the grounding deficit: models with $>88\%$ Level~2 accuracy fail catastrophically at Level~3, showing that grounding capability does not imply revision capability. \emph{Second}, reasoning-model optimization (GPT-5.2) does not confer advantage over instruction optimization (Claude Sonnet 4.6) on defeasible abduction, challenging the assumption that chain-of-thought training improves formal logical reasoning. \emph{Third}, the CoT ``overthinking'' effect is specific to structured formal tasks: it degrades performance on candidate-selection but has negligible effect where the task lies beyond model capability.
