...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: The full DeFAb dataset (409 Tier~0 evaluation instances, 324,511 Tier~1 instances, and 26,693 multi-tier instances, all in JSON), the generation pipeline, the extraction infrastructure for 14 knowledge sources, and the evaluation harness are released under the MIT license. Supplemental material includes the \texttt{README} with one-command reproduction instructions. Evaluated models are accessible via commercial APIs (Azure AI Foundry) and open-source vLLM hosting (CURC), both described in Section~\ref{subsec:evaluation}.
    \item[] Guidelines:
    \begin{itemize}
