...
The synthetic contamination control reproduces the same panel on the 35 structurally matched L3 synthetic instances at M4 (direct and CoT, 280 calls). Per-model accuracy drops uniformly: DeepSeek-R1 65.0\%~$\to$~48.6\% ($\Delta_{\mathrm{synth}} = +16.4$~pp), GPT-5.2-chat 47.5\%~$\to$~21.7\% ($+25.8$~pp), Claude~Sonnet~4.6 16.4\%~$\to$~2.9\% ($+13.5$~pp), Kimi-K2.5 14.2\%~$\to$~1.4\% ($+12.8$~pp); mean $+17.1$~pp. Decoder-failure rates rise to 97\% (Claude) and 99\% (Kimi). The uniformly positive sign is consistent with partial vocabulary-level memorization on naturalistic L3, but the cognitive load of unfamiliar predicates may also contribute. Even after gap correction the best-model L3 accuracy (48.6\%) remains far below the symbolic ceiling, so the structural-deficit conclusion is robust to the control.
