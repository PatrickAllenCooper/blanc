...
\paragraph{Models.} We evaluate five foundation models spanning three closed-source families and one open-source family at two scales: GPT-4o~\cite{openai2023gpt4}, Claude~3.5 Sonnet~\cite{anthropic2024claude}, Gemini~1.5 Pro~\cite{google2024gemini}, Qwen~2.5 72B (AWQ-quantized)~\cite{qwen2024qwen25}, and Qwen~2.5 32B (AWQ-quantized) as the smaller within-family comparator. As of February 2026, Qwen~2.5 72B is the highest-ranked open-source model on standard benchmarks, fitting within a single A100~80\,GB GPU under AWQ 4-bit quantization ($\sim$36\,GB VRAM). The inclusion of two Qwen~2.5 models at different scales (32B and 72B) enables within-family scaling analysis at matched quantization level. Where the analysis requires an alternative open-source family for cross-family comparison, we additionally evaluate Llama~3.3 70B (AWQ-INT4)~\cite{dubey2024llama3}, the current leading Llama model. All models are evaluated with greedy decoding (temperature $= 0$) and a standardized system prompt that presents the task without chain-of-thought scaffolding, except where noted.

\paragraph{Compute infrastructure.} Closed-source models are accessed via Azure OpenAI Service (GPT-4o) and direct vendor APIs (Claude, Gemini). Open-source models are served via a production vLLM~\cite{kwon2023vllm} inference server on CURC Alpine, University of Colorado Boulder's HPC cluster, on nodes equipped with NVIDIA A100 80\,GB GPUs. The vLLM server exposes an OpenAI-compatible REST API, enabling a uniform evaluation pipeline across all model families. Throughput on a single A100 with AWQ-quantized 72B models exceeds 500 tokens per second, enabling the full evaluation sweep within the CURC allocation window.
