...
\paragraph{Extension to formalized mathematics.} The Lakatosian connection identified in Sections~\ref{sec:intro} and~\ref{sec:related} suggests a natural extension: \textsc{DeFAb-Math}, a benchmark that applies the defeasible abduction framework to formalized mathematical libraries. The isomorphism is precise. Proven theorems with fully specified hypotheses are strict rules; weakened variants with dropped hypotheses are defeasible rules; counterexamples to the weakened statements are defeaters; and the conservativity requirement ensures that the counterexample blocks the weakened conjecture only in its exception region without invalidating the original theorem's domain. Mathlib (193K+ Lean~4 formalizations), MMLKG (65K+ Mizar theorems), and Sequencelib/OEIS provide the source theories. Lean's type checker replaces the polynomial-time defeasible derivation checker with an exact, sound, and complete verifier, and the verifier-in-the-loop fine-tuning pipeline transfers directly: Lean serves as the reward function for DPO and GRPO without approximation. At Level~3, the model receives a weakened theorem that makes an incorrect default prediction and must construct a minimal counterexample that blocks the prediction conservatively, preserving all instances where the original conditions hold. This is the Lakatosian conjecture--counterexample--refinement cycle, fully formalized and verifier-backed.

\paragraph{Defeasible conflict games for fine-tuning.} The current fine-tuning pipeline (Section~\ref{sec:finetuning}) constructs preferences by sampling model responses and scoring them with the verifier. An alternative is to generate preferences from \emph{adversarial game outcomes}, connecting the debate framework (Section~\ref{sec:debate}) to the preference optimization framework. Given a defeasible conflict where two rules produce contradictory conclusions, Player~A uses MCTS to construct a defeater that resolves the conflict in favor of Rule~A; Player~B does the same for Rule~B. Both defeaters are scored for resolution, conservativity, and novelty. The winner determines the preference pair for DPO. This produces training signal grounded in the structure of the defeasible theory itself rather than in model sampling errors, scales with the number of conflicts in the knowledge base (tens of thousands in UMLS alone), and connects naturally to self-play RLVR: the model plays both sides, the verifier scores both resolutions, and GRPO's group-relative advantage normalization concentrates learning on conflicts where the model produces mixed-quality defeaters. As the model improves, the games become harder, creating an automatic curriculum without manual scheduling.
