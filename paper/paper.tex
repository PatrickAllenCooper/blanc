...
\paragraph{Adversarial debate as a third evaluation paradigm.} The debate protocol of Section~\ref{sec:debate} introduces a qualitatively different evaluation mechanism: rather than measuring a model's accuracy on fixed instances, it measures robustness under adversarial proof perturbation. The MCTS proposal phase generates evaluation targets adaptively, focusing on the most ``interesting'' regions of the derivation space (those with high UCB1 reward). This complements both the static evaluation of Section~\ref{sec:results} and the gradient-based approach of Section~\ref{sec:finetuning}: debate generates challenging instances dynamically, fine-tuning internalises solutions, and direct evaluation provides the fixed benchmark for comparability across studies.

\paragraph{Limitations.} The Level~3 set contains 35 expert-authored instances; Wilson 95\% CI half-widths are approximately $\pm 15$--$17$ pp on aggregate accuracy and $\pm 25$--$33$ pp per domain, so per-domain subgroup inferences should be treated as descriptive. The M1 decoder operates approximately by design. Results are restricted to English-language theories from three domains. The fine-tuning pipeline of Section~\ref{sec:finetuning} is constrained by the current dataset size (409 instances); scaling to subsequent KB tiers (Table~\ref{tab:kbsummary}) will be necessary to distinguish genuine capability acquisition from overfitting. The synthetic contamination control (Section~\ref{subsec:synthetic}) mitigates but does not eliminate the contamination confound: while synthetic predicate names are verified against Common Crawl, the \emph{structural patterns} of defeasible theories (e.g., the pattern of a taxonomic hierarchy with inherited behavioral defaults and subclass exceptions) may themselves be memorized, even if the specific lexical content is novel. Demonstrating that models exploit structural contamination would require a separate analysis not attempted here.
