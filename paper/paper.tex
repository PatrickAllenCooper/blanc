...
Domain breakdown at Level~2 (Wilson 95\% CI half-width $\pm 3$--$4$ pp) reveals a consistent ordering: legal is the hardest domain for all models (Claude 72\%, DeepSeek-R1 67\%, GPT-5.2 57\%, Kimi 64\%), while biology is consistently easiest (82\%, 80\%, 68\%, 68\%). This ordering is stable across model families and reflects the denser rule structures and more complex defeasibility conditions of LKIF Core relative to the biology and materials KBs, rather than idiosyncratic knowledge gaps: no model performs substantially better on legal than others when controlling for overall accuracy level.
...
Domain breakdown (descriptive only; per-domain Level~3 $n \in \{9, 10, 16\}$ gives Wilson CI half-widths of $\pm 25$--$33$ pp): biology is consistently the easiest domain for all models, legal the hardest. DeepSeek-R1 achieves 72\% in biology, 56\% in legal, 62\% in materials; GPT-5.2 achieves 52\%, 44\%, 43\% respectively.
